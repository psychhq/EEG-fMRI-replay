...
path_participants <- file.path(dirname(path_root), 'highspeed-bids','forBIDS', "participants.tsv")
# path to the events.tsv files (according to BIDS):
path_events <- file.path(dirname(path_root), 'highspeed-bids','forBIDS', "*", "*", "func", "*events.tsv")
# path to data from the decoding analysis:
path_pred <- file.path(dirname(path_root), 'highspeed-decoding', "decoding", "*", "data", "*decoding.csv")
# path to the data from the mask thresholding:
path_thresh <- file.path(dirname(path_root), 'highspeed-decoding', "decoding", "*", "data", "*thresholding.csv")
# path to the data of the voxel patterns:
path_patterns <- file.path(dirname(path_root), 'highspeed-decoding', "decoding", "*", "data", "*voxel_patterns_union*.csv")
...
# datalad get data/decoding/decoding/sub-*/data/*_decoding.csv
...
# datalad get data/decoding/decoding/*/data/*thresholding.csv
dt_thresh = do.call(rbind, lapply(Sys.glob(path_thresh), data.table::fread))
# read all data from the mask thresholding:
# datalad get data/decoding/decoding/*/data/*voxel_patterns_union*.csv
files_patterns = Sys.glob(path_patterns)
files_patterns = files_patterns[!stringr::str_detect(files_patterns, "hpc")]
#dt_patterns = lapply(files_patterns, data.table::fread)
