...
ana_conditions = 'vfl_glm_activation'
...
def l1_GLM_pm(subject):
    ...
    for i, run_id in enumerate(run_list):
        ...
        events_file['trial_type'] = events_file['accuracy'].apply(lambda x: 'correct' if x == 1. else 'wrong')
    ...
    contrasts = {'correct_con': [], }
    for design_matrix in design_matrices:
        contrast_matrix = np.eye(design_matrix.shape[1])
        basic_contrasts = dict([(column, contrast_matrix[i])
                                for i, column in enumerate(design_matrix.columns)])
        correct_con = basic_contrasts['correct']
        for contrast_id in ['correct_con']:
...
# load the beta list
# correct_beta_list = [beta_list[i][0] for i in range(len(sub_list))]

# RAW T-MAP
for (con, beta_map) in zip(['correct_vfl'], [beta_list]):
