...
ana_conditions = 'vfl_glm_contrast_1'
...
def l1_GLM_pm(subject):
    ...
    for i, run_id in enumerate(run_list):
        ...
        # event types and onset for semantic stimuli
        for label, event in zip(label_list_sem, class_list_sem):
            events_file.loc[(events_file['textMarker'] == label), 'trial_type_sem'] = events_file.loc[
                (events_file['textMarker'] == label), 'accuracy'].apply(
                lambda x: event if x == 1. else 'wrong_sem')
        events_sem = events_file.loc[:, ['trial_type_sem', 'onset_semantic', 'resp_rt']]
        events_sem = events_sem.rename(
            columns={'trial_type_sem': 'trial_type', 'onset_semantic': 'onset', 'resp_rt': 'duration'})
        # event types and onset for response
        events_file.loc[events_file['resp_rt'] != 1, 'trial_type_resp'] = events_file.loc[
        ...
        events_resp = events_resp.rename(
            columns={'trial_type_resp': 'trial_type', 'onset_resp': 'onset', 'duration_resp': 'duration'})
        # concatenate all the events
        events = pd.concat((events_vis, events_sem, events_resp), axis=0)
    ...
    contrasts = {'face_vis': [], 'scissor_vis': [], 'zebra_vis': [], 'banana_vis': [],
                 'face_sem': [], 'scissor_sem': [], 'zebra_sem': [], 'banana_sem': [],
                 'face_contrast': [], 'scissor_contrast': [], 'zebra_contrast': [], 'banana_contrast': [],
                 'face_sem_contrast': [], 'scissor_sem_contrast': [], 'zebra_sem_contrast': [],
                 'banana_sem_contrast': [], 'all_visual': [], 'all_sem': []}
    for design_matrix in design_matrices:
        ...
        face_sem = basic_contrasts['face_sem']
        scissor_sem = basic_contrasts['scissor_sem']
        zebra_sem = basic_contrasts['zebra_sem']
        banana_sem = basic_contrasts['banana_sem']
        # visual contrast
        face_contrast = 3 * basic_contrasts['face'] - basic_contrasts['scissor'] - \
                        basic_contrasts['zebra'] - basic_contrasts['banana']
        scissor_contrast = 3 * basic_contrasts['scissor'] - basic_contrasts['face'] - \
                           basic_contrasts['zebra'] - basic_contrasts['banana']
        zebra_contrast = 3 * basic_contrasts['zebra'] - basic_contrasts['scissor'] - \
                         basic_contrasts['face'] - basic_contrasts['banana']
        banana_contrast = 3 * basic_contrasts['banana'] - basic_contrasts['scissor'] - \
                          basic_contrasts['zebra'] - basic_contrasts['face']
        # semantic contrast
        face_sem_contrast = 3 * basic_contrasts['face_sem'] - basic_contrasts['scissor_sem'] - \
                            basic_contrasts['zebra_sem'] - basic_contrasts['banana_sem']
        scissor_sem_contrast = 3 * basic_contrasts['scissor_sem'] - basic_contrasts['zebra_sem'] - \
                               basic_contrasts['face_sem'] - basic_contrasts['banana_sem']
        zebra_sem_contrast = 3 * basic_contrasts['zebra_sem'] - basic_contrasts['scissor_sem'] - \
                             basic_contrasts['face_sem'] - basic_contrasts['banana_sem']
        banana_sem_contrast = 3 * basic_contrasts['banana_sem'] - basic_contrasts['scissor_sem'] - \
                              basic_contrasts['face_sem'] - basic_contrasts['zebra_sem']
        # visual and semantic contrast
        all_visual = basic_contrasts['face'] + basic_contrasts['scissor'] + basic_contrasts['zebra'] + basic_contrasts[
            'banana']
        all_sem = basic_contrasts['face_sem'] + basic_contrasts['scissor_sem'] + basic_contrasts['zebra_sem'] + \
                  basic_contrasts['banana_sem']
        for contrast_id in ['face_vis', 'scissor_vis', 'zebra_vis', 'banana_vis',
                            'face_sem', 'scissor_sem', 'zebra_sem', 'banana_sem',
                            'face_contrast', 'scissor_contrast', 'zebra_contrast', 'banana_contrast',
                            'face_sem_contrast', 'scissor_sem_contrast', 'zebra_sem_contrast', 'banana_sem_contrast',
                            'all_visual', 'all_sem']:
...
face_sem_list = [beta_list[i][4] for i in range(len(sub_list))]
scissor_sem_list = [beta_list[i][5] for i in range(len(sub_list))]
zebra_sem_list = [beta_list[i][6] for i in range(len(sub_list))]
banana_sem_list = [beta_list[i][7] for i in range(len(sub_list))]
face_contrast_list = [beta_list[i][8] for i in range(len(sub_list))]
scissor_contrast_list = [beta_list[i][9] for i in range(len(sub_list))]
zebra_contrast_list = [beta_list[i][10] for i in range(len(sub_list))]
banana_contrast_list = [beta_list[i][11] for i in range(len(sub_list))]
face_sem_contrast_list = [beta_list[i][12] for i in range(len(sub_list))]
scissor_sem_contrast_list = [beta_list[i][13] for i in range(len(sub_list))]
zebra_sem_contrast_list = [beta_list[i][14] for i in range(len(sub_list))]
banana_sem_contrast_list = [beta_list[i][15] for i in range(len(sub_list))]
all_visual_list = [beta_list[i][16] for i in range(len(sub_list))]
all_sem_list = [beta_list[i][17] for i in range(len(sub_list))]

fun_mask = opj(path_mask, 'harvard_resample_4.nii')

# RAW T-MAP
for (con, beta_map) in zip(
        ['face_vfl', 'scissor_vfl', 'zebra_vfl', 'banana_vfl', 'face_sem', 'scissor_sem', 'zebra_sem', 'banana_sem',
         'face_contrast', 'scissor_contrast', 'zebra_contrast', 'banana_contrast',
         'face_sem_contrast', 'scissor_sem_contrast', 'zebra_sem_contrast', 'banana_sem_contrast',
         'all_visual_list', 'all_sem_list'],
        [face_list, scissor_list, zebra_list, banana_list, face_sem_list, scissor_sem_list, zebra_sem_list,
         banana_sem_list, face_contrast_list, scissor_contrast_list, zebra_contrast_list, banana_contrast_list,
         face_sem_contrast_list, scissor_sem_contrast_list, zebra_sem_contrast_list, banana_sem_contrast_list,
         all_visual_list, all_sem_list]):
