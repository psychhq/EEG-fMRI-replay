...
for path in [path_glm_l1, path_glm_l2, path_glm_roi]:
...
def l1_GLM_pm(subject):
    ...
    for cons, run_id in zip(cons_list, run_list):
        ...
        onset_file_all = pd.read_csv(onset_events, sep=',')
        ...
        design_matrix = make_first_level_design_matrix(
            frame_times=frame_times, events=None, drift_model=None,
        ...
        onset_file = pd.DataFrame(onset_file_all['probability'])
...
t_map_paired = second_level_model_paired.compute_contrast('Post vs Pre', second_level_stat_type='t',
                                                          output_type='stat')
t_image_path = opj(path_glm_l2, 'pair_ppi_fmri_rest_tmap.nii.gz')
t_map_paired.to_filename(t_image_path)

# MASKED THE T-MAP
t_map_mask_pair = threshold_img(t_map_paired, threshold=3, cluster_threshold=10,
                                two_sided=False, mask_img=mtl_mask, copy=True)
path_t_map = opj(path_glm_l2, "pair_ppi_fmri_rest_tmap_masked.nii.gz")
...
print("pre: entorhinal:", prob_thres, round(entor_t_test_pre.loc['T-test', 'T'],4), round(entor_t_test_pre.loc['T-test', 'p-val'],4), round(entor_t_test_pre.loc['T-test', 'cohen-d'],4),
      "\n post: entorhinal:", prob_thres, round(entor_t_test_post.loc['T-test', 'T'],4),
      round(entor_t_test_post.loc['T-test', 'p-val'],4),round(entor_t_test_post.loc['T-test', 'cohen-d'],4),
      "\n post-pre: entorhinal:", prob_thres, round(entor_t_test_paired.loc['T-test', 'T'],4),
      round(entor_t_test_paired.loc['T-test', 'p-val'],4), round(entor_t_test_paired.loc['T-test', 'cohen-d'],4),)

# print('post-pre: entorhinal:', prob_thres, entor_t_test_paired.loc['T-test', 'T'],
#       entor_t_test_paired.loc['T-test', 'p-val'])
...
print("pre: pmotor:", prob_thres, pmotor_t_test_pre.loc['T-test', 'T'], pmotor_t_test_pre.loc['T-test', 'p-val'], pmotor_t_test_pre.loc['T-test', 'cohen-d'],
      "\n post: pmotor:", prob_thres, pmotor_t_test_post.loc['T-test', 'T'], pmotor_t_test_post.loc['T-test', 'p-val'], pmotor_t_test_post.loc['T-test', 'cohen-d'],
      "\n post-pre: pmotor:", prob_thres, pmotor_t_test_paired.loc['T-test', 'T'],
      pmotor_t_test_paired.loc['T-test', 'p-val'], pmotor_t_test_paired.loc['T-test', 'cohen-d'],)

# print('post-pre: pmotor:', prob_thres, entor_t_test_paired.loc['T-test', 'T'],
#       entor_t_test_paired.loc['T-test', 'p-val'])
...
beta_entor.to_csv(opj(path_glm_roi, 'beta_entor_fmri_rest_ppi.csv'))
...
beta_pmotor = beta_pmotor.melt(value_vars=['pre_rest', 'post_rest'],
                               var_name='session', value_name='beta_value')
beta_pmotor.to_csv(opj(path_glm_roi, 'beta_pmotor_fmri_rest_ppi.csv'))
