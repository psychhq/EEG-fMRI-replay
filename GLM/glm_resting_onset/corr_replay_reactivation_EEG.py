...
path_glm_cor = opj(path_bids, 'derivatives', 'glm-l1', 'nilearn', 'rea-rep-corr')
...
def corrplot(data, sub):
    ...
    plt.savefig(opj(path_glm_cor, '%s event correlation matrix' % sub), dpi=400)
    plt.show()
...
#         correlation_list.append(tempa)
#         # save the data
#         consname = {0: {'cons': 'pre-fwd'}, 1: {'cons': 'pre-bwd'},
#                     2: {'cons': 'post-fwd'}, 3: {'cons': 'post-bwd'}, }
#         file_name = '%s_%s_rest_replay_events.tsv' % (subject, consname[i]['cons'])
#         write_path = opj(path_glm_cor, sub)
#         if not os.path.exists(write_path):
#             os.makedirs(write_path)
#         tempa.to_csv(opj(write_path, file_name), sep='\t', index=0)
#
#     prob_pre = (save_list[0].iloc[:, 1] + save_list[1].iloc[:, 1])
#     tempa_pre = pd.concat((save_list[0].iloc[:, 0], prob_pre), axis=1)
#     correlation_list.append(tempa_pre)
#     prob_post = (save_list[2].iloc[:, 1] + save_list[3].iloc[:, 1])
#     tempa_post = pd.concat((save_list[2].iloc[:, 0], prob_post), axis=1)
#     correlation_list.append(tempa_post)
...
#     # print(stats.pearsonr(onset_file_react_pre.iloc[:, 7], onset_file_react_post.iloc[:, 7]))
...
#         file['onset'] = file['onset'].apply(
#             lambda x: float(str(x).split('.')[0] + '.' + str(x).split('.')[1][:2]))
#         file = file.loc[:, ['onset', 'A', 'B', 'C', 'D', 'probability']]
...
#         tempa = temp.drop_duplicates(subset='onset').sort_values(by='onset').reset_index(drop=True)
#         correlation_list.append(tempa)
#     mini_tr = np.minimum(scan_tr_pre, scan_tr_post)
#     mini_time = int(int(mini_tr) * tr * 100)
#     correlation_frame = pd.DataFrame(np.array(pd.concat((correlation_list[0].loc[:mini_time,'probability'],
#                                                 correlation_list[1].loc[:mini_time,'probability'],
#                                                 correlation_list[2].loc[:mini_time,'probability'],
#                                                 correlation_list[3].loc[:mini_time,'probability'],
#                                                 correlation_list[4].loc[:mini_time,'probability'],
#                                                 correlation_list[5].loc[:mini_time,'probability'],
#                                                 correlation_list[6].loc[:mini_time, ['A', 'B', 'C', 'D', 'probability']],
#                                                 correlation_list[7].loc[:mini_time, ['A', 'B', 'C', 'D', 'probability']]),
#                                                axis=1)),
#                                      columns=['pre_fwd', 'pre_bwd', 'post_fwd', 'post_bwd',
#                                               'pre_all', 'post_all',
#                                               'pre_A', 'pre_B', 'pre_C', 'pre_D', 'pre_react',
#                                               'post_A', 'post_B', 'post_C', 'post_D', 'post_react'])
#     corrplot(data=correlation_frame, sub=sub)
#
#     pre_post_replay = stats.pearsonr(correlation_frame['pre_all'], correlation_frame['post_all'])[0]
#     pre_post_react = stats.pearsonr(correlation_frame['pre_react'], correlation_frame['post_react'])[0]
#     pre_replay_react = stats.pearsonr(correlation_frame['pre_all'], correlation_frame['pre_react'])[0]
#     post_replay_react = stats.pearsonr(correlation_frame['post_all'], correlation_frame['post_react'])[0]
#     impcorr = pd.DataFrame((pre_post_replay, pre_post_react, pre_replay_react, post_replay_react))
#     impcorr_list.append(impcorr)
#
#
# impcorr_df = [impcorr_list[i].loc[:,0] for i in range(len(sub_list))]
# impcorr_df = pd.DataFrame(impcorr_df)
# impcorr_df.columns=['pre_post_replay', 'pre_post_react', 'pre_replay_react', 'post_replay_react']
# impcorr_df.mean(axis=0)
#
#         # spm HRF kernel
#         # hkernel = spm_hrf(tr=1.3, oversampling=100)
#         # # convolve the HRF kernel and probability event
...
for subject in sub_list:
    ...
    for i, cons, tempa in zip([0, 2], ['pre', 'post'], [tempa_pre, tempa_post]):
        ...
        # resample BOLD signal
        resample_onset = pd.DataFrame(f(frame_times).T, columns=['HRF_onset_TR'])
        correlation_list.append(resample_onset)
    ...
    mini_tr = np.minimum(scan_tr_pre, scan_tr_post)
    mini_time = int(mini_tr)
    correlation_frame = pd.DataFrame(np.array(pd.concat((correlation_list[0].loc[:mini_time-1,'HRF_onset_TR'],
                                                         correlation_list[1].loc[:mini_time-1,'HRF_onset_TR'],
                                                         correlation_list[2].loc[:mini_time-1,'HRF_onset_TR'],
                                                         correlation_list[3].loc[:mini_time-1,'HRF_onset_TR']),
                                                         axis=1)),
                                     columns=['pre_replay', 'post_replay', 'pre_react', 'post_react'])
    corrplot(data=correlation_frame, sub=sub)

    pre_post_replay = stats.pearsonr(correlation_frame['pre_replay'], correlation_frame['post_replay'])[0]
    pre_post_react = stats.pearsonr(correlation_frame['pre_react'], correlation_frame['post_react'])[0]
    pre_replay_react = stats.pearsonr(correlation_frame['pre_replay'], correlation_frame['pre_react'])[0]
    post_replay_react = stats.pearsonr(correlation_frame['post_replay'], correlation_frame['post_react'])[0]
    impcorr = pd.DataFrame((pre_post_replay, pre_post_react, pre_replay_react, post_replay_react))
    impcorr_list.append(impcorr)
