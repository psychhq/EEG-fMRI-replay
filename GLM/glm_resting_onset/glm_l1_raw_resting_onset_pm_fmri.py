...
ana_conditions = 'raw_rest_pm-fmri-multimultiply'
sub_list = None
...
path_onset_fmri = opj(path_bids, 'derivatives', 'resting-replay-onset', 'raw_replay_fmri')
...
path_onset_events = glob.glob(opj(path_onset_fmri, '*_resting_fmri_event.csv'))
# list
...
def l1_GLM_pm(subject):
    ...
    templates = dict(
        ...
        onset_events=opj(path_onset_fmri, 'sub-{subject}_{cons}_resting_fmri_event.tsv'),
    ...
    for cons, run_id in zip(cons_list, run_list):
        ...
        name = ['reactivation_fMRI_%s' % cons]
        ...
        # print(stats.pearsonr(design_matrices[0].loc[:,0],design_matrices[1].loc[:,0])
    ...
    reactivation_pre = basic_contrasts['reactivation_fMRI_pre'] # it is the regressor name, to form the contrast name
    ...
    basic_contrasts = dict([(column, contrast_matrix[i])
                            for i, column in enumerate(design_matrix.columns)])
    reactivation_post = basic_contrasts['reactivation_fMRI_post']
...
for (con, beta_map) in zip(['pre', 'post'], [pre, post]):
    ...
    t_image_path = opj(path_glm_l2, 'fmri_rea_tmap_%s.nii.gz' % con)
    t_map.to_filename(t_image_path)
    ...
    t_map_thres, threshold = threshold_stats_img(stat_img=t_map,
                                                 ...
                                                 cluster_threshold=0,
                                                 two_sided=True)
    t_thres_image_path = opj(path_glm_l2, 'fmri_rea_tmap_%s_thres.nii.gz' % con)
...
t_maps_paired = second_level_model_paired.compute_contrast('Post vs Pre',
                                                           ...
                                                           output_type='stat')
t_image_path = opj(path_glm_l2, 'fmri_rea_paired-tmap_post-pre.nii.gz')
...
t_maps_paired_thres, threshold = threshold_stats_img(stat_img=t_maps_paired,
                                                     ...
                                                     cluster_threshold=0,
                                                     two_sided=True)
t_thres_image_path = opj(path_glm_l2, 'fmri_rea_paired-tmap_thres_post-pre.nii.gz')
