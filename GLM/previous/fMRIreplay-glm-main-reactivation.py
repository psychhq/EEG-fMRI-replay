...
project_name = 'fmrireplay-glm-rea'
...
path_confounds = glob.glob(opj(path_fmriprep,'*','func', '*reactivation*confounds_timeseries.tsv'))
path_events = glob.glob(opj(path_bids ,'*','func', '*rea*events.tsv')) 
path_func = glob.glob(opj(path_fmriprep ,'*','func', '*reactivation*space-T1w*preproc_bold.nii.gz'))
path_anat = glob.glob(opj(path_fmriprep ,'*','anat', '*_desc-preproc_T1w.nii.gz'))
path_wholemask = glob.glob(opj(path_fmriprep ,'*','func', '*reactivation*space-T1w*brain_mask.nii.gz'))
...
templates = dict(
    confounds=opj(path_fmriprep,'{subject_id}' , 'func','*reactivation*confounds_timeseries.tsv'),
    events=opj(path_bids,'{subject_id}', 'func','*rea*events.tsv'),
    func=opj(path_fmriprep,'{subject_id}', 'func','*reactivation*space-T1w*preproc_bold.nii.gz'),
    anat=opj(path_fmriprep,'{subject_id}', 'anat','{subject_id}_desc-preproc_T1w.nii.gz'),
    wholemask=opj(path_fmriprep,'{subject_id}','func','*reactivation*space-T1w*brain_mask.nii.gz'),
)
