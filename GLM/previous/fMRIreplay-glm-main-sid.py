...
from fMRIreplay_glm_functions_sid import (
...
project_name = 'fmrireplay-glm-sid'
...
path_confounds = glob.glob(opj(path_fmriprep,'*','func', '*sid*confounds_timeseries.tsv'))
path_events = glob.glob(opj(path_bids ,'*','func', '*sid*events.tsv')) 
path_func = glob.glob(opj(path_fmriprep ,'*','func', '*sid*space-T1w*preproc_bold.nii.gz'))
path_anat = glob.glob(opj(path_fmriprep ,'*','anat', '*_desc-preproc_T1w.nii.gz'))
path_wholemask = glob.glob(opj(path_fmriprep ,'*','func', '*sid*space-T1w*brain_mask.nii.gz'))
...
templates = dict(
    confounds=opj(path_fmriprep,'{subject_id}' , 'func','*sid*confounds_timeseries.tsv'),
    events=opj(path_bids,'{subject_id}', 'func','*sid*events.tsv'),
    func=opj(path_fmriprep,'{subject_id}', 'func','*sid*space-T1w*preproc_bold.nii.gz'),
    anat=opj(path_fmriprep,'{subject_id}', 'anat','{subject_id}_desc-preproc_T1w.nii.gz'),
    wholemask=opj(path_fmriprep,'{subject_id}','func','*sid*space-T1w*brain_mask.nii.gz'),
)
...
# check the correct trials from sid task and transform to the event list
