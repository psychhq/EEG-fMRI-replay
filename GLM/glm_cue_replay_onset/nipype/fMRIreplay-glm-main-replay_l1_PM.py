...
path_glm_l1 = opj(path_bids, 'derivatives', 'glm-l1-pm')
...
identical = [26, 10, 7, 19, 8, 18, 24, 29, 35, 39, 41, 32, 14] # 13
delete_list = np.unique(
    np.hstack([loweeg, incomplete, extremeheadmotion, identical]))[::-1]
...
# selectfiles.inputs.subject_id = '04'
...
susan.get_node('median').interface.mem_gb = 2
susan.get_node('mask').interface.num_threads = 1
susan.get_node('mask').interface.mem_gb = 2
...
susan.get_node('merge').interface.mem_gb = 2
susan.get_node('multi_inputs').interface.num_threads = 1
susan.get_node('multi_inputs').interface.mem_gb = 2
susan.get_node('smooth').interface.num_threads = 1
susan.get_node('smooth').interface.mem_gb = 2
...
# l1design.inputs.flags = {'mthresh': float('-inf')}
#     ,
#                          'volt': 1}
...
# cont01 = ['forcor', 'T', condition_names,  [1, 0, 0]]
# cont02 = ['backcor', 'T', condition_names,  [0, 1, 0]]
# cont03 = ['allreplay', 'T', condition_names, [1, 1, 0]]
# contrast_list = [cont01, cont02, cont03]

# contrasts for parametric modulation
condition_names = ['forward_correct', 'forward_correctxrating_for^1',
                   'backward_correct', 'backward_correctxrating_back^1']
# contrast
cont01 = ['forcor_rating', 'T', condition_names, [0, 1, 0, 0]]
cont02 = ['backcor_rating', 'T', condition_names, [0, 0, 0, 1]]
cont03 = ['forcor', 'T', condition_names, [1, 0, 0, 0]]
cont04 = ['backcor', 'T', condition_names, [0, 0, 1, 0]]
cont05 = ['allreplay', 'T', condition_names, [1, 0, 1, 0]]
cont06 = ['allreplay_rating', 'T', condition_names, [0, 1, 0, 1]]
