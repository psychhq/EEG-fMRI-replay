...
ana_conditions = 'raw_pm-mean-8-orth-middle-mask'
sub_list = None
...
path_raw = opj(path_bids, 'derivatives', 'replay-onset', 'raw_replay')
...
path_onset_events = glob.glob(opj(path_raw, '*', '*_4_nodemean_middle_cue_replay_*_events.tsv'))
...
def l1_GLM_pm(subject):
    ...
    templates = dict(
        ...
        onset_events=opj(path_raw, 'sub-{subject}',
                         '{subject}_4_nodemean_middle_cue_replay_onset_run-{run}_events.tsv'),
    ...
    for i, run_id in enumerate(run_list):
        ...
        # concatenate the HRF-Convolved probability
        ...
        #                                   index=np.arange(int(scan_tr)) * time_repetition)
        # design_matrix_fin = pd.concat(
    ...
    return replay_beta_list, ms_beta_list
    # return ms_beta_list
...
replay_beta_list = [beta_list[i][0][0] for i in range(len(sub_list))]
ms_beta_list = [beta_list[i][1][0] for i in range(len(sub_list))]

# ms_beta_list = [beta_list[i][0] for i in range(len(sub_list))]
...
# for (con, beta_map) in zip(['ms'],[ms_beta_list]):
#     second_level_input = beta_map
#     design_matrix_l2 = pd.DataFrame(
#         [1] * len(second_level_input),
#         columns=['intercept'],
