...
def asd_label(subject,data):
    data['pic_label'] = None
    cond_1 = data['Correct_Label'] == 'A'
    cond_2 = data['Correct_Label'] == 'B'
    cond_3 = data['Correct_Label'] == 'C'
    cond_4 = data['Correct_Label'] == 'D'
    if subject == 'sub-04':
        data['pic_label'] = np.select([cond_1, cond_2, cond_3, cond_4], 
                                      ['keyboard', 'basketballhoop','basketball','girl'], 
                                      'Other')
    elif subject == 'sub-05':
        data['pic_label'] = np.select([cond_1, cond_2, cond_3, cond_4], 
                                      ['girl', 'shoe','keyboard','basketballhoop'], 
                                      'Other')
    elif subject == 'sub-06':
        data['pic_label'] = np.select([cond_1, cond_2, cond_3, cond_4], 
                                      ['house', 'girl','basketball','keyboard'], 
                                      'Other')
    elif subject == 'sub-07':
        data['pic_label'] = np.select([cond_1, cond_2, cond_3, cond_4], 
                                      ['clock', 'shoe','keyboard','watch'], 
                                      'Other')

def cvfold_asd(trials):
    if trials >= 0 and trials <= 8 :
        return 1
    elif trials > 8 and trials <= 17 :
        return 2
    elif trials > 17 and trials <= 26 :
        return 3
    elif trials > 26 and trials <= 35 :
        return 4
    elif trials > 35 and trials <= 44 :
        return 5
    elif trials > 44 and trials <= 53 :
        return 6
    elif trials > 53 and trials <= 63 :
        return 7
    elif trials > 63 and trials <= 73 :
        return 8
...
class TaskData:
    def __init__(
        ...
        if task == 'ASD-cue':
            ...
            self.num_vol_run[4] = num_vol_run_5
            self.num_vol_run[5] = num_vol_run_6
    ...
    def define_trs(self):
        ...
        if self.task == 'VFL' or self.task == "SID":
            # add the number of run volumes to the tr indices:
            run_volumes = (self.events["run"] - 1) * self.num_vol_run_1
        if self.task == 'ASD-cue':
            # add the number of run volumes to the tr indices:
...
path_bids = opj(path_root, "fmrireplay-bids", "OLD-BIDS")
...
path_glm_asd = opj(path_bids, "derivatives", "fmrireplay-glm-asd")
path_level1_asd = opj(path_glm_asd, "l1pipeline")
...
bold_delay_task_list = np.arange(1,10)

# for test
subject=sub_list[1]
mask_index=mask_index_list[0]
bold_delay_task=bold_delay_task_list[8]
# In[def]
...
def decoding(subject, mask_index,bold_delay_task): 
    ...
    # paths to all events files of the current subject:
    path_events_asd = opj(path_bids, subject, "func", "*asd-cue*events.tsv")
    # dl.get(glob.glob(path_events)) #can't run it, and i don't know the meaning of the data
    path_events_asd = sorted(glob.glob(path_events_asd), key=lambda f: os.path.basename(f))
    # import events file and save data in dataframe:
    df_events_asd = pd.concat(
        (pd.read_csv(f, sep="\t") for f in path_events_asd), ignore_index=True
    )
    asd_label(subject,df_events_asd)
    rename_asd = {'onset_value' : 'onset',
                  'pic_label' : 'stim_label'}
    df_events_asd = df_events_asd.rename(columns=rename_asd)
    df_events_asd['trials'] = np.arange(len(df_events_asd))
    df_events_asd['fold'] = df_events_asd.apply(lambda x: cvfold_asd(x['trials']),axis=1)
    ...
    # load the arbitrary sequence decoding mri task files:
    path_func_task_asd = opj(path_level1_asd, "smooth", subject, "*", "*task*nii.gz")
    path_func_task_asd = sorted(glob.glob(path_func_task_asd), key=lambda f: os.path.basename(f))
    logging.info("found %d functional mri task files" % len(path_func_task_asd))
    logging.info("paths to functional mri task files:\n%s" % pformat(path_func_task_asd))
    ...
    # load visual mask:
    mask_vis = [image.load_img(i).get_data().astype(int) for i in copy.deepcopy(path_mask_vis_task[16:18])]
    logging.info("successfully loaded one visual mask file!")
    # load mtl mask:
    mask_mtl = [image.load_img(i).get_data().astype(int) for i in copy.deepcopy(path_mask_mtl_task[16:18])]
    logging.info("successfully loaded one mtl mask file!")
    ...
    # load temporal mask:
    mask_temporal = [image.load_img(i).get_data().astype(int) for i in copy.deepcopy(path_mask_temporal_task[16:18])]
    logging.info("successfully loaded one temporal mask file!")
    # load prefrontal mask:
    mask_prefrontal = [image.load_img(i).get_data().astype(int) for i in copy.deepcopy(path_mask_prefrontal_task[16:18])]
    logging.info("successfully loaded one prefrontal mask file!")   
    # load prefrontal mask:
    mask_mixed = [image.load_img(i).get_data().astype(int) for i in copy.deepcopy(path_mask_mixed_task[16:18])]
    logging.info("successfully loaded one prefrontal mask file!")   
    # load prefrontal mask:
    mask_tem_pre = [image.load_img(i).get_data().astype(int) for i in copy.deepcopy(path_mask_tem_pre_task[16:18])]
    logging.info("successfully loaded one prefrontal mask file!")   
    ...
    # logging.info("loading successful!")
    logging.info("loading %d functional task runs ..." % len(path_func_task_asd))
    data_task_asd = [image.load_img(i) for i in path_func_task_asd]
    logging.info("loading successful!")
    mask_list = [
        mask_vis,
        mask_mtl,
    ...
    #     num_vol_run_2=np.size(data_task_sid[1]._data,axis=3),
    #     )
    # test_ASD_cue_peak = TaskData(
    #     events=df_events_asd,
    #     task="ASD-cue",
    #     # trial_type="stimulus",
    #     bold_delay=bold_delay_task,
    #     interval=1,
    #     name="test-ASD_cue_peak",
    #     num_vol_run_1=np.size(data_task_asd[0]._data,axis=3),
    #     num_vol_run_2=np.size(data_task_asd[1]._data,axis=3),
    #     num_vol_run_3=np.size(data_task_asd[2]._data,axis=3),
    #     num_vol_run_4=np.size(data_task_asd[3]._data,axis=3),
    #     num_vol_run_5=np.size(data_task_asd[4]._data,axis=3),
    #     num_vol_run_6=np.size(data_task_asd[5]._data,axis=3),
    #     )
    
    test_ASD_cue_long = TaskData(
        events=df_events_asd,
        task="ASD-cue",
        # trial_type="stimulus",
        bold_delay=0,
        interval=9,
        name="test-ASD_cue_long",
        num_vol_run_1=np.size(data_task_asd[0]._data,axis=3),
        num_vol_run_2=np.size(data_task_asd[1]._data,axis=3),
        num_vol_run_3=np.size(data_task_asd[2]._data,axis=3),
        num_vol_run_4=np.size(data_task_asd[3]._data,axis=3),
    ...
    class_labels = list(np.unique(test_ASD_cue_long.events['stim_label']))
    ...
    train_VFL_peak.trs[-1]=train_VFL_peak.trs[-3]
    # test_VFL_long.trs[-2]=test_VFL_long.trs[-3]
    # test_ASD_cue_long.trs[-1]=test_ASD_cue_long.trs[-5]
    # test_ASD_cue_long.trs[-2]=test_ASD_cue_long.trs[-5]
    # test_ASD_cue_long.trs[-3]=test_ASD_cue_long.trs[-5]
    # test_ASD_cue_long.trs[-4]=test_ASD_cue_long.trs[-5]
    n_fold = 8
    data_list = []
    ...
    for fold, run in zip(folds, runs):
        ...
        masked_data_test = [masking.apply_mask(i, mask_run) for i in data_task_asd]
    ...
    pre_list_filter = pre_list_append[(pre_list_append["class"] != 'other') &
                                      ...
                                      (pre_list_append["test_set"] == 'test-ASD_cue_long')]
    ...
    plt.suptitle('%s decoding probability of VFL-ASD (%1.0f th TR peak) %s' % (subject,bold_delay_task,mask_name))
    plt.savefig(opj(path_out_figs,'%s decoding probability of VFL-ASD (%1.0f th TR peak) %s.png' % (subject,bold_delay_task,mask_name)),
    ...
    def pred_acc(stim,pred_label):
        if (stim == pred_label):
            ...
            return 0
    pre_list_filter =  pre_list_filter[pre_list_filter['stim'] == pre_list_filter['classifier']]
    pre_list_filter['pred_acc'] = pre_list_filter.apply(lambda x: pred_acc(x['stim'],x['pred_label']), axis=1)
    pred_acc_mean = [np.mean(pre_list_filter[pre_list_filter['seq_tr'] ==  seq_tr]['pred_acc']) 
                     for seq_tr in np.arange(1,10)]
    pred_acc_mean = pd.DataFrame(pred_acc_mean,columns=['pred_acc'])
    pred_tr = pd.DataFrame(np.arange(1,10),columns=['pred_tr'])
    pred_acc_mean_sub = pd.concat([pred_acc_mean, pred_tr], axis=1)
    pred_acc_mean_sub['subject'] = subject
    pred_acc_mean_sub['bold_delay_task'] = bold_delay_task
    pred_acc_mean_sub['mask_name'] = mask_name
...
pred_acc = np.vstack(pred_acc)
# pred_acc = np.transpose(pred_acc)
pred_acc = pd.DataFrame(pred_acc,columns=['pred_acc','pred_tr','subject','bold_delay_task','brain_mask'])
pred_acc_mean = pred_acc.groupby(['pred_tr','bold_delay_task','brain_mask'],as_index=False)['pred_acc'].mean()

pred_acc = [pred_acc_mean for pred_acc_mean in pred_acc_mean.groupby('brain_mask',as_index=False)]
pred_acc_mat=[np.zeros((9,9)),np.zeros((9,9)),np.zeros((9,9)),np.zeros((9,9)),np.zeros((9,9)),np.zeros((9,9))]
for n in mask_index_list:
    mask_name = pred_acc[n][0]
    for i,j in zip((pred_acc[n][1]['pred_tr']),(pred_acc[n][1]['bold_delay_task'])):
        pred_acc_mat[n][i-1,j-1] = pred_acc[n][1][(pred_acc[n][1]['pred_tr']== i)& (pred_acc[n][1]['bold_delay_task']==j)]['pred_acc']
    # plot the decoded state space
    plt.figure(dpi=400)
    plt.xticks(fontsize=10)
    plt.imshow(pred_acc_mat[n], aspect='auto', interpolation='none',
                origin='lower',cmap='hot')
    plt.title('Decoding Accuracy of VFL-ASD %s.png' % mask_name)
    plt.xlabel('Decoding TR in VFL (train time)')
    plt.ylabel('Decoding TR in ASD (test time)')
    plt.xticks(range(9),(1,2,3,4,5,6,7,8,9))
    plt.yticks(range(9),(1,2,3,4,5,6,7,8,9))
    plt.colorbar()
    for ASDTR in range (9):
        for VFLTR in range (9):
            T_text = pred_acc_mat[n][
                VFLTR,ASDTR]
            plt.text(ASDTR,VFLTR,'{:0.2f}'.format(
                T_text), ha='center',va='center',color='royalblue')
    plt.savefig(opj(path_out_all,'decoding accuracy of VFL-ASD %s.png' % mask_name),
                dpi = 400,bbox_inches = 'tight')
    plt.show()
