...
def TDLM(probability_matrix,all_sequence,condition,sub,isub):
    rp = []
    # print(isub,sub)    
    ...
    for i in range(len(all_sequence)):
        ...
        if condition == 'sf':
            sf[isub,0,i,1:] = betas_2GLM[0,:]
            sb[isub,0,i,1:] = betas_2GLM[1,:]
        elif condition == 'sb':
            sf[isub,1,i,1:] = betas_2GLM[0,:]
            sb[isub,1,i,1:] = betas_2GLM[1,:]
...
# nSubj = len(suball)
nSubj = len(sub_list)
# the nan matrix for forward design matrix (36 subjects * 2 directions * 23 shuffles * 7 timelags)
...
def decoding(mask_index, bold_delay_task):
    ...
    for (isub,sub) in enumerate(sub_list):
        ...
        TDLM(probability_matrix=pred_matrix_51_seq,all_sequence=all_sequence,condition='sf',sub=sub,isub=isub)
        TDLM(probability_matrix=pred_matrix_52_seq,all_sequence=all_sequence,condition='sb',sub=sub,isub=isub)
    
    # In[PLOT the FIGURE]
        # mean the real sequence and permutation sequences respectively
        sf_sequence_51 = sf[isub,0,0,:]
        temp_per = sf[isub,0,1:,1:]
        temp_per = abs(temp_per)
        temp_per = np.amax(temp_per,axis=1)
        sf_permutation = matlab_percentile(temp_per,95)
        
        sb_sequence_51 = sb[isub,0,0,:]
        temp_per = sb[isub,0,1:,1:]
        temp_per = abs(temp_per)
        temp_per = np.amax(temp_per,axis=1)
        sb_permutation = matlab_percentile(temp_per,95)
        
        dif = sf_sequence_51 - sb_sequence_51
        temp_per = sf[isub,0,1:,1:]-sb[isub,0,1:,1:]
        temp_per = abs(temp_per)
        temp_per = np.amax(temp_per,axis=1)
        ...
        sf_sequence_52 = sf[isub,1,0,:]
        temp_per = sf[isub,1,1:,1:]
        temp_per = abs(temp_per)
        temp_per = np.amax(temp_per,axis=1)
        sf_permutation = matlab_percentile(temp_per,95)
        
        sb_sequence_52 = sb[isub,1,0,:]
        temp_per = sb[isub,1,1:,1:]
        temp_per = abs(temp_per)
        temp_per = np.amax(temp_per,axis=1)
        sb_permutation = matlab_percentile(temp_per,95)
        
        dif = sf_sequence_52 - sb_sequence_52
        temp_per = sf[isub,1,1:,1:]-sb[isub,1,1:,1:]
        temp_per = abs(temp_per)
        temp_per = np.amax(temp_per,axis=1)
        dif_permutation = matlab_percentile(temp_per,95)
