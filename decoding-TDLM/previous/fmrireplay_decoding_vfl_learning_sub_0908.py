...
def classlabel(Marker):
    if Marker == 11 or Marker == 21 or Marker == 31 or Marker == 51:
        return 'girl'
    elif Marker == 12 or Marker == 22 or Marker == 32  or Marker == 52:
        return 'scissors'
    elif Marker == 13 or Marker == 23 or Marker == 33:
        return 'zebra'
    elif Marker == 14 or Marker == 24 or Marker == 34:
        return 'banana'
...
delete_list = [30,27,25,22,15,11,4]
...
def decoding(subject):
    ...
    # learning file
    path_events_learning = opj(path_bids, subject, "func", "*learning*events.tsv")
    path_events_learning = sorted(glob.glob(path_events_learning), key=lambda f: os.path.basename(f))
    # import events file and save data in dataframe:
    df_events_learning = pd.concat(
        (pd.read_csv(f, sep="\t") for f in path_events_learning), ignore_index=True
    )
    df_events_learning['stim_label'] = df_events_learning.apply(lambda x: classlabel(x.ProbeMarker), axis = 1)

    del df_events_learning['Unnamed: 0']
    # del df_events_learning['start']
    ...
    mask_hippocampus = [image.load_img(i) for i in copy.deepcopy(path_mask_hippocampus_task[8:12])]
    ...
    test_rep_long = TaskData(
        ...
        num_vol_run_3 = np.size(data_task_rep[2]._data,axis=3),
        num_vol_run_4 = 0,
        )
    test_sets = [
        # test_VFL_peak,
    ...
    data_detrend_test_stand = standardize(data_detrend_test)
    tr_onset = np.unique(test_rep_long.stim_trs)
    visual_data_trial = [data_detrend_test_stand[tr_onset[i]:tr_onset[i]+13,] for i in range(len(tr_onset))]
    visual_data_trial = np.array(visual_data_trial)
    visual_data = visual_data_trial.mean(axis=2)
    visual_data = visual_data.mean(axis=0)
    ...
    data_detrend_test_stand = standardize(data_detrend_test)
    tr_onset = np.unique(test_rep_long.stim_trs)
    hippocampus_data_trial = [data_detrend_test_stand[tr_onset[i]:tr_onset[i]+13,] for i in range(len(tr_onset))]
    hippocampus_data_trial = np.array(hippocampus_data_trial)
    hippocampus_data = hippocampus_data_trial.mean(axis=2)
    hippocampus_data = hippocampus_data.mean(axis=0)
    ...
    return[hippocampus_data, visual_data]
...
# sb_sequence_51 = sf[:,0,0,:].mean(axis=0)
# temp_per = sf[:,0,1:,1:].mean(axis=0)
# temp_per = abs(temp_per)
# temp_per = np.amax(temp_per,axis=1)
# sb_permutation = matlab_percentile(temp_per,95)
...
# sb_sequence_52 = sf[:,1,0,:].mean(axis=0)
# temp_per = sf[:,1,1:,1:].mean(axis=0)
# temp_per = abs(temp_per)
# temp_per = np.amax(temp_per,axis=1)
# sb_permutation = matlab_percentile(temp_per,95)
...
# sb_sequence = sf[:,:,0,:].mean(axis=1).mean(axis=0)
# temp_per = sf[:,:,1:,1:].mean(axis=1).mean(axis=0)
# temp_per = abs(temp_per)
# temp_per = np.amax(temp_per,axis=1)
# sb_permutation = matlab_percentile(temp_per,95)
...
# dif = sf_sequence_52 - sb_sequence_52
# temp_per = (sf[:,:,1:,1:]-sf[:,:,1:,1:]).mean(axis=0)
# temp_per = temp_per.mean(axis=0)
# temp_per = abs(temp_per)
# temp_per = np.amax(temp_per,axis=1)
# dif_permutation = matlab_percentile(temp_per,95)
