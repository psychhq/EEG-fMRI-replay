...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  1 13:55:26 2022
...
class TaskData:
    ...
    def predict(self, clf, fold_list):
        ...
        df["id"] = np.repeat(self.events["participant"].unique(), num_pred)
...
bold_delay_task = 4  # 4, 5 or 6 secs  ???
...
def decoding(subject):
    ...
    tmaps_masked = [np.multiply(mask_vis, i) for i in copy.deepcopy(tmaps_vfl)]
    ...
    #     logging.info("plotting raw tmap %s (%d of %d)" % (path, i + 1, len(path_tmap_sid)))
    #     path_save = opj(path_out_figs, "%s_%s_run-%02d_tmap_raw.png" % (subject, taskname, i + 1))
    #     plotting.plot_roi(
    #         path,
    #         anat,
    ...
    #     sys.exit("Values > 1 in the anatomical ROI!")
    # # get combination of anatomical mask and t-map
    # tmaps_masked = [np.multiply(mask_vis, i) for i in copy.deepcopy(tmaps_sid)]
    # # masked tmap into image like object:
    ...
    #     path_save = opj(path_out_masks, filename)
    #     logging.info(
    #         "saving final mask %s (%d of %d)" % (path_save, i + 1, len(masks_final_sid)))
    #     path.to_filename(path_save)
    #     path_save = opj(
    #         path_out_figs, "%s_%s_run-%02d_visual_final_mask.png" % (subject, taskname, i + 1))
    #     logging.info(
    #         "plotting final mask %s (%d of %d)" % (path_save, i + 1, len(masks_final_sid)))
    #     plotting.plot_roi(
    #         path,
    #         anat,
    #         title=os.path.basename(path_save),
...
data_list_all = Parallel(n_jobs=32)(
...
pre_list_append= pre_list[0] + pre_list[1] + pre_list[2] + pre_list[3]
...
pre_list_mean = pre_list_filter.groupby('id')['pred_acc'].mean()
...
pre_list_filter = pre_list_append[(pre_list_append["class"] != 'other') &
                                  (pre_list_append["classifier"] != 'log_reg') &
                                  (pre_list_append["test_set"] == 'test-VFL_peak')]
pred_class_mean = [pre_list_filter[(pre_list_filter['stim'] == class_label) &
                                 (pre_list_filter['classifier'] == class_label)].groupby('id')['probability'].mean()
                 for class_label in class_labels]
pred_class_mean = np.matrix(pred_class_mean)
pred_class_mean_meant = pd.DataFrame(np.transpose(pred_class_mean),index=[4,5,6,7],columns=[class_labels])
pred_class_mean_meantt = pd.melt(pred_class_mean_meant,value_vars=[class_labels],var_name=['Classifier'],value_name='Probability')
sns.set_theme(style="whitegrid")
plt.figure(dpi=400,figsize=(10,5))
sns.barplot(x="Classifier", y="Probability", data=pred_class_mean_meantt)
plt.suptitle('decoding probability of visual functional localizer (%1.0fth TR peak)' % bold_delay_task) 
plt.savefig(opj(path_out_all,'decoding probability of visual functional localizer in %1.0fth TR peak.png' % bold_delay_task),
...
pred_class_mean = [[pre_list_filter[(pre_list_filter['stim'] == class_label) &
                                   ...
                                   (pre_list_filter['classifier'] == class_label)].groupby('id')['probability'].mean()
                 for class_label in class_labels] for seq_tr in np.arange(1,10)]
pred_class_mean = np.array(pred_class_mean)
...
plt.suptitle('decoding probability of visual functional localizer (%1.0fth TR peak)' % bold_delay_task) 
plt.savefig(opj(path_out_all,'decoding probability of visual functional localizer (%1.0fth TR peak)' % bold_delay_task),
            dpi = 400, bbox_inches = 'tight')
plt.show()
...
for run in runs:
    ...
    # get the feature selection mask of the current run:
    # if mask_label == "cv":
    mask_run = masks_final_sid[runs.index(run)]
    # elif mask_label == "cv_hpc":
    #     mask_run = path_mask_hpc_task[runs.index(run)]
    # extract smoothed fMRI data from the mask for the cross-validation fold:
    ...
    for clf_name, clf in clf_set.items():
        ...
        logging.info("classifier: %s" % clf_name)
        # fit the classifier to the training data:
        train_SID_peak.zscore(signals=data_detrend, fold_list=train_runs)
        # get the example labels:
        train_stim = copy.deepcopy(train_SID_peak.stim[train_SID_peak.runs != run])
        # replace labels for single-label classifiers:
        if clf_name in class_labels:
            # replace all other labels with other
            train_stim = ["other" if x != clf_name else x for x in train_stim]
        ...
        for test_set in test_sets:
            logging.info("testing on test set %s" % test_set.name)
            test_set.zscore(signals=data_detrend, fold_list=test_runs)
            if test_set.data_zscored.size < 0:
                continue
            # create dataframe containing classifier predictions:
            df_pred = test_set.predict(clf=clf, fold_list=test_runs)
            # add the current classifier as a new column:
            df_pred["classifier"] = np.repeat(clf_name, len(df_pred))
            # add a label that indicates the mask / training regime:
            df_pred["mask"] = np.repeat(mask_label, len(df_pred))
...
# In[DECODING ON RESTING STATE DATA]:
...
#     path_save = opj(
#         path_out_masks, "{}_task-rest_mask-{}.nii.gz".format(subject, mask_label)
#     )
...
#     # add the participant id to the dataframe:
#     df_patterns["id"] = np.repeat(
#         df_events["subject"].unique(), len(train_odd_peak.stim)
#     )
#     # add the mask label:
...
#     file_paths = [
#         opj(path_out_data, "{}_voxel_patterns_{}_{}".format(subject, mask_label, i))
#         for i in df_patterns["label"].unique()
#     ]
#     # save the final dataframe to a .csv-file:
#     [
#         i.to_csv(j + ".csv", sep=",", index=False)
#         for (i, j) in zip(df_pattern_list, file_paths)
#     ]
#     # save only the voxel patterns as niimg-like objects
#     [
#         masking.unmask(
#             X=i.loc[:, voxel_labels].to_numpy(), mask_img=masks_union_nii
#         ).to_filename(j + ".nii.gz")
#         for (i, j) in zip(df_pattern_list, file_paths)
#     ]
...
#             # create a dataframe with the probabilities of each class:
#             df_rest_pred = pd.DataFrame(pred_rest_proba[t], columns=classes_names)
#             # get the number of predictions made:
...
# # create file path to save the dataframe:
# file_path = opj(path_out_data, "{}_decoding.csv".format(subject))
# # save the final dataframe to a .csv-file:
