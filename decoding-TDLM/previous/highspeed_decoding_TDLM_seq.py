...
# for n,sub in zip(range(len(suball)),suball):
    ...
    select_list = np.array([5,11,17,23,29,35,41,47])
    pre_list_append = []
    pre_list_append = pd.concat([pre_list[i] for i in select_list], axis=0)
    pre_list_filter = pre_list_append[(pre_list_append["tITI"] == 2.048)]
    pre_list_filter['class_index'] = pre_list_filter['class'].map(lambda a : stim_index(a))
    pre_list_filter_sort = pre_list_filter.sort_values(by=['class_index','trial'])
    ...
    serial1 = pd.concat([pd.DataFrame(pred_matrix[13*i:13*(i+1),int(test_seq_long.seq2048[i,0])-1]  
                                      ) for i in range(len(test_seq_long.trial2048))],axis=1)
    serial2 = pd.concat([pd.DataFrame(pred_matrix[13*i:13*(i+1),int(test_seq_long.seq2048[i,1])-1]  
                                      ) for i in range(len(test_seq_long.trial2048))],axis=1)
    serial3 = pd.concat([pd.DataFrame(pred_matrix[13*i:13*(i+1),int(test_seq_long.seq2048[i,2])-1]  
                                      ) for i in range(len(test_seq_long.trial2048))],axis=1)
    serial4 = pd.concat([pd.DataFrame(pred_matrix[13*i:13*(i+1),int(test_seq_long.seq2048[i,3])-1]  
                                      ) for i in range(len(test_seq_long.trial2048))],axis=1)
    serial5 = pd.concat([pd.DataFrame(pred_matrix[13*i:13*(i+1),int(test_seq_long.seq2048[i,4])-1]  
                                      ) for i in range(len(test_seq_long.trial2048))],axis=1)
    x=np.arange(1,14)
    plt.figure(dpi=400)
    plt.xticks(fontsize=10)
    plt.plot(x, serial1.mean(axis=1), color='darkred', linestyle='-', linewidth=1, label='Serial event 1')
    plt.plot(x, serial2.mean(axis=1), color='orangered', linestyle='-', linewidth=1, label='Serial event 2')
    plt.plot(x, serial3.mean(axis=1), color='gold', linestyle='-', linewidth=1, label='Serial event 3')
    plt.plot(x, serial4.mean(axis=1), color='darkgreen', linestyle='-', linewidth=1, label='Serial event 4')
    plt.plot(x, serial5.mean(axis=1), color='dodgerblue', linestyle='-', linewidth=1, label='Serial event 5')
    plt.legend(bbox_to_anchor=(1.05, 0), loc=3, borderaxespad=0) 
    plt.xlabel('time(TR)') 
    plt.ylabel('probablity')
    plt.ylim(0,1)
    plt.savefig(('C:\\Users\\NINGMEI\\Desktop\\Resting\\Classifier Probability-%s.png' % sub),
            dpi = 400,bbox_inches = 'tight')
    plt.show()
    
    seq_pred_matrix = np.transpose(np.vstack((np.reshape(serial1.values,195,order = "F"),
                                              np.reshape(serial2.values,195,order = "F"),
                                              np.reshape(serial3.values,195,order = "F"),
                                              np.reshape(serial4.values,195,order = "F"),
                                              np.reshape(serial5.values,195,order = "F"))))
    
    scipy.io.savemat('seq_pred_matrix.mat',{'seq_pred_matrix':seq_pred_matrix}) 
    '''
    ========================================================================
    ...
    sf = np.full([maxLag+1],np.nan)
    sb = np.full([maxLag+1],np.nan)
    ...
    for i in range(len(test_seq_long.trial2048)): 
        ...
        real_sequence = [test_seq_long.seq2048[i,:].astype(np.int32),]
        ...
        # for j in range(len(new_all_sequence)):
        #     # real sequence
        #     rp = new_all_sequence[j,]
    rp=np.array((1,2,3,4,5))
    ...
    # sf_sequence = sf.mean(axis=0)
    # temp_per = sf[:,1:,1:].mean(axis=0)
    # temp_per = abs(temp_per)
    # sf_permutation =np.amax(np.amax(temp_per, axis=1),axis=0)
    dif = sf - sb
    ...
    plt.xticks(fontsize=10)
    x=np.arange(0,nbins)
    l1=plt.plot(x,sf,color='turquoise',linestyle='-',marker = 'o',label='sequential trial')
    # l2=plt.plot(x,sf_permutation,color='silver',linestyle='--',label='permutation threshold')
    ...
    plt.xticks(fontsize=10)
    x=np.arange(0,nbins)
    l1=plt.plot(x,sb,color='turquoise',linestyle='-',marker = 'o',label='sequential trial')
    # l2=plt.plot(x,sf_permutation,color='silver',linestyle='--',label='permutation threshold')
    # plt.axhline(y=sf_permutation,color='silver',linestyle='--')
    ...
    plt.xticks(fontsize=10)
    x=np.arange(0,nbins)
    l1=plt.plot(x,dif,color='turquoise',linestyle='-',marker = 'o',label='sequential trial')
    # l2=plt.plot(x,sf_permutation,color='silver',linestyle='--',label='permutation threshold')
    # plt.axhline(y=sf_permutation,color='silver',linestyle='--')
    ...
    plt.ylabel('squenceness')
    plt.legend()
    plt.savefig(('C:\\Users\\NINGMEI\\Desktop\\Resting\\Forward-Backward replay-%s.png' % sub),
                dpi = 400,bbox_inches = 'tight')
    plt.show()
'''
========================================================================
