...
# bold_delay_task=bold_delay_task_list[3]
...
def decoding(subject,bold_delay_task): 
    ...
    test_SID_long.trs[-1]=test_SID_long.trs[-5]
    test_SID_long.trs[-2]=test_SID_long.trs[-5]
    test_SID_long.trs[-3]=test_SID_long.trs[-5]
    test_SID_long.trs[-4]=test_SID_long.trs[-5]
    n_fold = 8
    data_list = []
    ...
    for fold, run in zip(folds, runs):
        ...
        masked_data_test = [masking.apply_mask(i, mask_run) for i in data_task_sid]
    ...
    # plt.title('%s_decoding probability of visual functional localizer in %1.0f th TR peak' % (subject,bold_delay_task))
    # plt.xlabel("label")
    # plt.ylabel("probability")
    # plt.savefig(opj(path_out_figs,'%s_decoding probability of visual functional localizer in %1.0f th TR peak.png') % (subject,bold_delay_task),
    ...
    # plt.suptitle('%s decoding probability of visual functional localizer (%1.0f th TR peak)' % (subject,bold_delay_task))
    # plt.savefig(opj(path_out_figs,'%s decoding probability of visual functional localizer (%1.0f th TR peak)' % (subject,bold_delay_task)),
    #             dpi = 400,bbox_inches = 'tight')
    # plt.show() 
    ...
    for i in range(len(x_data)):
     	plt.bar(x_data[i], y_data[i])
    plt.axhline(y=0.125,color='silver',linestyle='--')
    plt.title('%s_decoding probability of VFL-SID in %1.0f th TR peak %s' % (subject,bold_delay_task,mask_name))
    plt.xlabel("label")
    plt.ylabel("probability")
    plt.savefig(opj(path_out_figs,'%s_decoding probability of VFL-SID in %1.0f th TR peak %s.png') % (subject,bold_delay_task,mask_name),
                dpi = 400,bbox_inches = 'tight')
    plt.show()
    ...
    fig.text(0, 0.5, 'Probability(%)', va='center', rotation='vertical')
    plt.tight_layout(pad=2.5, w_pad=0.5, h_pad=2)
    plt.suptitle('%s decoding probability of VFL-SID (%1.0f th TR peak) %s' % (subject,bold_delay_task,mask_name))
    plt.savefig(opj(path_out_figs,'%s decoding probability of VFL-SID (%1.0f th TR peak) %s.png' % (subject,bold_delay_task,mask_name)),
                dpi = 400,bbox_inches = 'tight')
    plt.show() 
