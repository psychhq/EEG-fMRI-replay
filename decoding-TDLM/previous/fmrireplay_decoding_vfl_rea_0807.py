...
def classlabel(Marker):
    if Marker == 11 or Marker == 21 or Marker == 41:
        return 'girl'
    elif Marker == 12 or Marker == 22 or Marker == 42:
        return 'car'
...
class TaskData:
    ...
    def define_trs(self):
        ...
        if self.task == 'VFL' or self.task == 'REA':
...
def decoding(subject, mask_index, bold_delay_task):
    ...
    # reactivation file
    path_events_rea = opj(path_bids, subject, "func", "*rea*events.tsv")
    # dl.get(glob.glob(path_events)) #can't run it, and i don't know the meaning of the data
    path_events_rea = sorted(glob.glob(path_events_rea), key=lambda f: os.path.basename(f))
    # import events file and save data in dataframe:
    df_events_rea = pd.concat(
        (pd.read_csv(f, sep="\t") for f in path_events_rea), ignore_index=True
    )
    df_events_rea['stim_label'] = df_events_rea.apply(lambda x: classlabel(x.Marker), axis = 1)
    del df_events_rea['Unnamed: 0']
    ...
    path_func_task_rea = opj(path_level1_rea, "smooth", subject, "*", "*task*nii.gz")
    path_func_task_rea = sorted(glob.glob(path_func_task_rea), key=lambda f: os.path.basename(f))
    ...
    data_task_rea = [image.load_img(i) for i in path_func_task_rea]

    mask_list = [mask_temporal,mask_visual,mask_tem_pre,mask_prefrontal,mask_mixed,mask_mtl]
    mask_name_list= ['mask_temporal','mask_vis','mask_tem_pre','mask_prefrontal','mask_mixed','mask_mtl']
    ...
    #     num_vol_run_4 = np.size(data_task_vfl[3]._data,axis=3),
    # )
    test_rea_reak = TaskData(
        events=df_events_rea,
        task="REA",
        # trial_type="stimulus",
        bold_delay=bold_delay_task,
        interval=1,
        name="test-rea_peak",
        num_vol_run_1=np.size(data_task_rea[0]._data,axis=3),
        num_vol_run_2=np.size(data_task_rea[1]._data,axis=3),
        num_vol_run_3=np.size(data_task_rea[2]._data,axis=3),
        num_vol_run_4=0,
        )
    
    test_rea_long = TaskData(
        events=df_events_rea,
        task="REA",
        # trial_type="stimulus",
        bold_delay=0,
    ...
    for fold, run in zip(folds, runs):
        ...
        masked_data_test = [masking.apply_mask(i, mask_run) for i in data_task_rea]
    ...
    #                                   (pre_list_append["test_set"] == 'test-rea_peak') &
    ...
    # pred_acc_mean_sub = pd.DataFrame([[pred_acc_mean],[subject],[bold_delay_task],[mask_name]])
    ...
    plt.axhline(y=0.25,color='silver',linestyle='--')
    plt.title('%s decoding probability of VFL-REA (%1.0f th TR peak) %s' % (subject,bold_delay_task,mask_name))
    plt.xlabel("label")
    plt.ylabel("probability")
    plt.savefig(opj(path_out_figs,'%s decoding probability of VFL-REA (%1.0f th TR peak) %s.png' % (subject,bold_delay_task,mask_name)),
    ...
    pre_list_filter = pre_list_append[(pre_list_append["class"] != 'other') &
                                      (pre_list_append["classifier"] != 'log_reg') &
                                      (pre_list_append["test_set"] == 'test-rea_long')]
    pred_class_mean = [[np.mean(pre_list_filter[(pre_list_filter['stim'] == class_label) 
    ...
    fig, ax = plt.subplots(nrows = 1, ncols = 4,sharex=True, sharey=True,figsize=(10,3),dpi=400)
    for j, class_label in enumerate(class_labels):
        ...
        ax[j].plot(x_data,y_data,color='black',linestyle='-')
    ...
    plt.suptitle('%s multi-TR decoding probability of VFL-REA (%1.0f th TR peak) %s' % (subject,bold_delay_task,mask_name))
    plt.savefig(opj(path_out_figs,'%s multi-TR decoding probability of VFL-REA (%1.0f th TR peak) %s.png' % (subject,bold_delay_task,mask_name)),
                dpi = 400,bbox_inches = 'tight')
    plt.show() 
...
pred_acc_mean = pred_acc[pred_acc['subject']=='sub-09']
...
for n in mask_index_list:
    ...
    plt.title('Decoding Accuracy of VFL-Reactivation %s.png' % mask_name)
    plt.xlabel('Decoding TR in VFL (train time)')
    plt.ylabel('Decoding TR in reactivation (test time)')
    plt.xticks(range(9),(1,2,3,4,5,6,7,8,9))
    plt.yticks(range(9),(1,2,3,4,5,6,7,8,9))
    plt.colorbar()
    for REATR in range (9):
        for VFLTR in range (9):
            T_text = pred_acc_mat[n][
                VFLTR,REATR]
            plt.text(REATR,VFLTR,'{:0.2f}'.format(
                T_text), ha='center',va='center',color='royalblue')
    plt.savefig(opj(path_out_all,'decoding accuracy of VFL-REA %s.png' % mask_name),
                dpi = 400,bbox_inches = 'tight')
    plt.show()
