...
def decoding(subject,mask_index,bold_delay_task): 
    ...
    # prediction probability
    pred_acc_mean_sub = pd.DataFrame([[pred_acc_mean],[subject],[mask_index],[bold_delay_task]])
    pred_acc_mean_sub_all.append(pred_acc_mean_sub)
    ...
    for j, class_label in enumerate(class_labels):
        ...
        raw = math.floor(j/4)
        col = j-4*raw 
        ax[raw][col].set_ylim(ymax=0.4)
        ax[raw][col].set_xticks(range(1,len(whole_TR)+1),labels=whole_TR)
        ax[raw][col].set_yticks([0,0.1,0.2,0.3,0.4],labels=[0,10,20,30,40])
        ax[raw][col].plot(x_data,y_data,color='black',linestyle='-')
        ax[raw][col].axhline(y=0.125,color='silver',linestyle='--')
        ax[raw][col].set_title('%s' % class_label) 
    fig.text(0.5, 0, 'Time from stimulus onset (TRs)', ha='center')
...
data_list_all = Parallel(n_jobs=50)(
