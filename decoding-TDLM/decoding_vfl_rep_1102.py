...
def decoding(subject):
    ...
    pred_matrix_51_seq = temp_pred_prob(pre_list_append, 51., picList)
    # backward mental simulation
    pred_matrix_52_seq = temp_pred_prob(pre_list_append, 52., picList)
    ...
    sf_permutation_51 = matlab_percentile(temp_per, 95)

    sb_sequence_51 = sf[0, 0, :, 1]
    temp_per = sf[0, 1:, 1:, 1]
    temp_per = abs(temp_per)
    temp_per = np.amax(temp_per, axis=1)
    sb_permutation_51 = matlab_percentile(temp_per, 95)

    dif_51 = sf_sequence_51 - sb_sequence_51
    temp_per = sf[0, 1:, 1:, 0] - sf[0, 1:, 1:, 1]
    temp_per = abs(temp_per)
    temp_per = np.amax(temp_per, axis=1)
    dif_permutation_51 = matlab_percentile(temp_per, 95)

    sf_sequence_52 = sf[1, 0, :, 0]
    temp_per = sf[1, 1:, 1:, 0]
    temp_per = abs(temp_per)
    temp_per = np.amax(temp_per, axis=1)
    sf_permutation_52 = matlab_percentile(temp_per, 95)

    sb_sequence_52 = sf[1, 0, :, 1]
    temp_per = sf[1, 1:, 1:, 1]
    temp_per = abs(temp_per)
    temp_per = np.amax(temp_per, axis=1)
    sb_permutation_52 = matlab_percentile(temp_per, 95)

    dif_52 = sf_sequence_52 - sb_sequence_52
    temp_per = sf[1, 1:, 1:, 0] - sf[1, 1:, 1:, 1]
    temp_per = abs(temp_per)
    temp_per = np.amax(temp_per, axis=1)
    dif_permutation_52 = matlab_percentile(temp_per, 95)

    # plot the sequenceness
    TDLM_fig_each('forward', sf_sequence_51, sf_permutation_51, '1-2-3-4',
                  path_out_figs, task_bold_delay, mask_name)
    TDLM_fig_each('backward', sb_sequence_51, sb_permutation_51, '1-2-3-4',
                  path_out_figs, task_bold_delay, mask_name)
    TDLM_fig_each('for-back', dif_51, dif_permutation_51, '1-2-3-4',
                  path_out_figs, task_bold_delay, mask_name)
    # plot the sequenceness
    TDLM_fig_each('forward', sf_sequence_52, sf_permutation_52, '4-3-2-1',
                  path_out_figs, task_bold_delay, mask_name)
    TDLM_fig_each('backward', sb_sequence_52, sb_permutation_52, '4-3-2-1',
                  path_out_figs, task_bold_delay, mask_name)
    TDLM_fig_each('for-back', dif_52, dif_permutation_52, '4-3-2-1',
                  path_out_figs, task_bold_delay, mask_name)
...
data_list_all = [all_results_list[i][0] for i in range(len(sub_list))]
...
pre_list_append.to_csv(opj(path_out_replay, 'replay_decoding.csv'), index=False)
...
# new data inut
pred_matrix_51 = [all_results_list[i][1] for i in range(len(sub_list))]
pred_matrix_52 = [all_results_list[i][2] for i in range(len(sub_list))]

# plot the temporal dynamic probability
Temporal_probability('1-2-3-4', pred_matrix_51, long_interval, nSubj, path_out_replay)
Temporal_probability('4-3-2-1', pred_matrix_52, long_interval, nSubj, path_out_replay)

# In[all subject TDLM replay]:
mask_name = 'mtl'
sf_list = np.array([all_results_list[i][3] for i in range(len(sub_list))])
# all subject TDLM in forward mental simulation condition
# forward replay
sf_sequence_51 = sf_list[:, 0, 0, :, 0]
sf_permutation = matlab_percentile(
    np.amax(abs(sf_list[:, 0, 1:, 1:, 0].mean(axis=0)), axis=1), 100)
# backward replay
sb_sequence_51 = sf_list[:, 0, 0, :, 1]
sb_permutation = matlab_percentile(
    np.amax(abs(sf_list[:, 0, 1:, 1:, 1].mean(axis=0)), axis=1), 100)
# forward replay - backward replay
dif = sf_sequence_51 - sb_sequence_51
dif_permutation = matlab_percentile(np.amax(abs(
    (sf_list[:, 0, 1:, 1:, 0] - sf_list[:, 0, 1:, 1:, 1]).mean(axis=0)), axis=1), 100)

# plot the sequenceness
TDLM_fig('forward', sf_sequence_51, sf_permutation, '1-2-3-4',
         path_out_meanreplay, task_bold_delay, mask_name)
TDLM_fig('backward', sb_sequence_51, sb_permutation, '1-2-3-4',
         path_out_meanreplay, task_bold_delay, mask_name)
TDLM_fig('for-back', dif, dif_permutation, '1-2-3-4',
         path_out_meanreplay, task_bold_delay, mask_name)
...
sf_permutation = matlab_percentile(
    np.amax(abs(sf_list[:, 1, 1:, 1:, 0].mean(axis=0)), axis=1), 100)
# backward replay
sb_sequence_52 = sf_list[:, 1, 0, :, 1]
sb_permutation = matlab_percentile(
    np.amax(abs(sf_list[:, 1, 1:, 1:, 1].mean(axis=0)), axis=1), 100)
# forward replay - backward replay
dif = sf_sequence_52 - sb_sequence_52
dif_permutation = matlab_percentile(np.amax(abs(
    (sf_list[:, 1, 1:, 1:, 0] - sf_list[:, 1, 1:, 1:, 1]).mean(axis=0)), axis=1), 100)
# plot the sequenceness
TDLM_fig('forward', sf_sequence_52, sf_permutation, '4-3-2-1',
         path_out_meanreplay, task_bold_delay, mask_name)
TDLM_fig('backward', sb_sequence_52, sb_permutation, '4-3-2-1',
         path_out_meanreplay, task_bold_delay, mask_name)
TDLM_fig('for-back', dif, dif_permutation, '4-3-2-1',
         path_out_meanreplay, task_bold_delay, mask_name)
